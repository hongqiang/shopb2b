...
@Controller("adminAlipayTradeController")
@RequestMapping({ "${adminPath}/payment_plugin/alipay_trade" })
public class AlipayTradeController extends BaseController{
	@Resource(name = "alipayTradePlugin")
	private AlipayTradePlugin alipayTradePlugin;
	...
	public String install(RedirectAttributes redirectAttributes) {
		if (!this.alipayTradePlugin.getIsInstalled()) {
			PluginConfig localPluginConfig = new PluginConfig();
			localPluginConfig.setPluginId(this.alipayTradePlugin.getId());
	...
	public String uninstall(RedirectAttributes redirectAttributes) {
		if (this.alipayTradePlugin.getIsInstalled()) {
			PluginConfig localPluginConfig = this.alipayTradePlugin.getPluginConfig();
	...
	public String setting(ModelMap model) {
		...
		return "/admin/alipayTrade/setting";
	}
	...
	public String update(String paymentName, String seller_email, String partner, String key,
		...
		PluginConfig pluginConfig = this.alipayTradePlugin.getPluginConfig();
